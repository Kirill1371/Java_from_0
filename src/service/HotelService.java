        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(filePath))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                try {
                    ...
                    // List<String> stays = parts.length > 2 ? Arrays.asList(parts[2].trim().split(";")) : new ArrayList<>();
                    // List<String> services = parts.length > 3 ? Arrays.asList(parts[3].trim().split(";")) : new ArrayList<>();

                    Guest newGuest = new Guest(name);
                    // newGuest.getStays().addAll(stays.stream().map(Stay::new).toList());
                    // newGuest.getServices().addAll(services.stream().map(Service::new).toList());
